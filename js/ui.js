    ...
    shopsToDisplay.forEach(shop => {
        ...
        card.innerHTML = `
            ...
            <div class="p-6">
                ...
                <button class="block w-full text-center py-3 btn-primary text-sm tracking-widest view-details-btn" data-shop-id="${shop.id}">${window.locales[currentLang]?.viewDetailsBtn || 'VIEW DETAILS'}</button>
            </div>
        `;
...
window.showShopDetail = function(shopId) {
    ...
    container.innerHTML = `
        <div class="relative">
            <div class="h-64 shop-detail-header rounded-t-xl"></div>
            <button class="close-modal-btn absolute top-4 right-4 bg-white/20 hover:bg-white/40 text-white rounded-full p-2 transition">
        ...
        <div class="p-8">
            <div class="flex justify-between items-start mb-6">
                <div>
                    <span class="tag px-3 py-1 rounded-full text-sm font-medium mb-2 inline-block">${type}</span>
                    <h2 class="text-4xl font-bold text-gray-900">${name}</h2>
                </div>
                <div class="flex gap-3">
                    <button onclick="window.handleShopAction('${shop.id}', 'favorite')" class="p-3 border border-gray-200 rounded-xl hover:bg-red-50 transition group">
                    ...
                    <button onclick="window.handleShopAction('${shop.id}', 'rate')" class="p-3 border border-gray-200 rounded-xl hover:bg-yellow-50 transition group">
                    ...
                    <button onclick="window.handleShopAction('${shop.id}', 'report')" class="p-3 border border-gray-200 rounded-xl hover:bg-blue-50 transition group">
